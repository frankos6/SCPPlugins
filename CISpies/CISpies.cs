...
namespace SCPPlugins.CISpies
{
    ...
    public class CISpies : Plugin<Config>
    {
        ...
        private void ServerOnRespawningTeam(RespawningTeamEventArgs ev)
        {
            ...
            if (Server.SessionVariables["SpyRespawned"].Equals(true))
                return; //check if a spy spawned this round
        ...
        private static void ServerOnEndingRound(EndingRoundEventArgs ev)
        {
            ...
            if (Player.List.Any(x => x.SessionVariables["IsSpy"].Equals(true)) &&
                ev.LeadingTeam == LeadingTeam.FacilityForces) ev.IsRoundEnded = false;
            //assign the right team the win if only spies are alive
            if (!Player.List.Any(x => x.IsNTF && x.SessionVariables["IsSpy"].Equals(false)))
            {
                ev.LeadingTeam = Round.EscapedDClasses == 0 && Player.List.Any(x => x.IsScp)
                    ? LeadingTeam.Anomalies
                    : LeadingTeam.ChaosInsurgency;
                ev.IsRoundEnded =
                    Player.List.All(x =>
                        x.Role != RoleTypeId.ClassD &&
                        x.Role != RoleTypeId.Scientist); //dont end round if d/scientists are alive
            }
        }
        ...
        private static void ServerOnRoundStarted()
        {
            ...
            Player.List.ToList().ForEach(player => { player.SessionVariables["IsSpy"] = false; });
        }
        ...
        public static void RevealPlayer(Player player)
        {
            ...
            player.Role.Set(RoleTypeId.ChaosRifleman, SpawnReason.ForceClass,
                RoleSpawnFlags.None); //Change class to CI Rifleman, keep inventory, don't use default spawnpoint
        ...
        private static IEnumerator<float> NotifySpiesCoroutine()
        {
            while (true)
            {
                ...
                foreach (var spy in spies) spy.ShowHint("Remember that you are a CI spy!", 5f);
